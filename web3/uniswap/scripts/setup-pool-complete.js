...
// Network configurations - Using PoolModifyLiquidityTest which can initialize + add liquidity
const NETWORKS = {
    sepolia: {
...
async function main() {
    ...
    console.log(`\n🚀 Creating Pool & Adding Liquidity on ${networkName}`);
    console.log(`   Using PoolModifyLiquidityTest: ${config.poolModifyTest}\n`);
    ...
    // Check balances
    const [usdcBal, wethBal, ethBal] = await Promise.all([
    ...
    console.log("\n2️⃣ Initializing pool and adding liquidity...");
    console.log("   (PoolModifyLiquidityTest will initialize the pool automatically)\n");

    // Start with reasonable liquidity - 1000 units
    const liquidityDelta = hre.ethers.parseUnits("1000", 18);
    ...
    console.log(`   Liquidity: ${hre.ethers.formatUnits(liquidityDelta, 18)} units`);
    ...
    try {
        ...
        // Try with smaller liquidity
        if (e.message.includes("insufficient") || e.message.includes("balance") || e.message.includes("reverted")) {
            console.log(`\n⚠️  Trying with minimal liquidity (100 units)...`);

            params.liquidityDelta = hre.ethers.parseUnits("100", 18);
            ...
            if (retryReceipt.status === 1) {
                console.log(`\n✅ Pool created with minimal liquidity!`);
